...
vector<Book> FuzzySearchBookByName(vector<Book>& books, const string& keyword);		//根据名称模糊查找
...
Book* UpdateBookByISBN(vector<Book>& books, const string& ISBN, const Book& newinformation);	//根据ISBN修改图书
Book* UpdateBookByName(vector<Book>& books, const string& name, const Book& newinformation);	//根据名称修改图书
...
bool LoadBooksFromFile(vector<Book>& books, const string& filename);	//从文件中加载图书
bool SaveBooksToFile(vector<Book>& books, const string& filename);		//将图书存储到文件中